...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		if (GameData.platinumLogo)
			...
			logoBl.frames = Paths.getSparrowAtlas('Funkin_Logo');
		logoBl.scale.set(0.84, 0.84);
		logoBl.antialiasing = FlxG.save.data.antialiasing;
		logoBl.animation.addByPrefix('bump', 'logo bumin', 24);
	...
	override function update(elapsed:Float)
	{
		...
		// workaround without the need of importing
	...
	function deleteCoolText()
	{
		...
		{
			fadeAway(textGroup.members[i]);
		}
	}

	function fadeAway(object:Dynamic)
	{
		FlxTween.tween(object, {y: object.y + 40, alpha: 0}, 0.4, {
			ease: FlxEase.sineOut,
			onComplete: function(twn:FlxTween)
			{
				credGroup.remove(object, true);
				textGroup.remove(object, true);
			}
		});
	}
	...
	override function beatHit()
	{
		...
		if (!skippedIntro)
		{
			switch (curBeat)
			{
				...
				// credTextShit.screenCenter();
				case 5:
					createCoolText(['associated with', 'the game']);
				case 7:
