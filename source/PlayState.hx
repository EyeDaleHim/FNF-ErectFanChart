...
class PlayState extends MusicBeatState
{
	...
	var multiDrain:String;

	var darkBG:FlxSprite;
	var lightsDadbattle:FlxTypedGroup<FlxSprite>;
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			case 'senpai' | 'roses' | 'thorns':
				dialogue = CoolUtil.coolTextFile(Paths.txt('$leSong/$leSong' + 'Dialogue'));
		}
		...
		add(bgSprites);
		lightsDadbattle = new FlxTypedGroup<FlxSprite>();
			...
			case 'spookeez' | 'monster' | 'south' | 'judgement':
				{
					...
					vignette.antialiasing = antialiasing;
			...
			default:
				{
					...
					darkBG = new FlxSprite(-FlxG.width, -FlxG.height).makeGraphic(Std.int(FlxG.width * 6), Std.int(FlxG.height * 6), FlxColor.BLACK);
					darkBG.alpha = 0;
					bgSprites.add(darkBG);

					for (i in 0...4)
					{
						var stageLight:FlxSprite = new FlxSprite(-70 + (390 * i), -180).loadGraphic(Paths.image('stageLight'));
						stageLight.alpha = 0;
						stageLight.colorTransform.alphaMultiplier = 0.4;
						lightsDadbattle.add(stageLight);
					}
				}
		}
		...
		iconP1.x = healthBar.x + healthBar.width - 14;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (daNoteType == SEAL || daNoteType == HALO)
					multiDrain = '| Health Drain Multi: $healthMulti';
				else
					multiDrain = '';
		...
		for (i in 0...4)
		{
			...
			if (((!noSongFadeArrows.contains(SONG.song.toLowerCase()) && isStoryMode) || !debugCutscene))
			{
				...
				switch (curStage)
				{
					...
					default:
						if (player == 1)
							FlxTween.tween(babyArrow, {y: pos, alpha: 1}, 1, {ease: FlxEase.circOut, startDelay: 1.1 - (0.2 * i)});
						else
							FlxTween.tween(babyArrow, {y: pos, alpha: 1}, 1, {ease: FlxEase.circOut, startDelay: 0.5 + (0.2 * i)});
				}
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (health <= 0)
		{
			if (#if (tester || debug) FlxG.keys.pressed.SHIFT #else true #end)
			{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (downscroll)
				{
					...
					if (daNote.isSustainNote)
					{
						...
						if (daNote.y - daNote.offset.y * daNote.scale.y + daNote.height >= strum.members[daNote.noteData].y
							&& (!daNote.mustPress || (daNote.wasGoodHit || (daNote.prevNote.wasGoodHit && !daNote.canBeHit))))
						{
							var swagRect = new FlxRect(0, 0, daNote.frameWidth, daNote.frameHeight);
							swagRect.height = (strum.members[daNote.noteData].y - daNote.y) / daNote.scale.y;
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					{
						case 0:
							enemy.playAnim('singLEFT' + altAnim, daBool);
							camOffset[0] = -15;
						case 1:
							enemy.playAnim('singDOWN' + altAnim, daBool);
							camOffset[1] = 15;
						case 2:
							enemy.playAnim('singUP' + altAnim, daBool);
							camOffset[1] = -15;
						case 3:
							enemy.playAnim('singRIGHT' + altAnim, daBool);
							camOffset[0] = 15;
					}

					if (curSong == 'South' && !daNote.isSustainNote)
						flashLight(true, daNote.noteData);
					...
					if (!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection
						...
						&& curSong == 'Dadbattle')
					{
						FlxTween.cancelTweensOf(lightsDadbattle.members[daNote.noteData]);
						var colors:Array<FlxColor> = [
							FlxColor.fromRGB(194, 75, 153, 102), FlxColor.fromRGB(0, 255, 255, 102),
							  FlxColor.fromRGB(18, 250, 0, 102), FlxColor.fromRGB(249, 57, 63, 102)
						];
						FlxTween.color(lightsDadbattle.members[daNote.noteData], 0.2, FlxColor.fromRGB(255, 255, 255, 102), colors[daNote.noteData]);
						lightsDadbattle.members[daNote.noteData].alpha = 0.4;
					}
					...
					// if ur so far from 0%, decrease it hard
					if (health > 1.5)
					{
						if (daNote.isSustainNote)
							health -= 0.06 * 0.25;
			...
			for (i in 0...playerStrums.length)
			{
				notes.forEachAlive(function(daNote:Note)
				{
					...
					if (playerStrums.members[i].animation.curAnim.name == 'confirm'
						&& daNote.strumTime > Conductor.songPosition + Conductor.stepCrochet + (Conductor.safeZoneOffset * 2)
						&& daNote.strumTime < Conductor.songPosition + Conductor.stepCrochet + (Conductor.safeZoneOffset * 16)
						&& daNote.mustPress
						&& !daNote.canBeHit)
					{
						if (holdKey[i] > Conductor.crochet / 1000)
						{
					...
					if (directionList.contains(daNote.noteData))
					{
						for (coolNote in possibleNotes)
						{
							if (coolNote.noteData == daNote.noteData
								&& Math.abs(daNote.strumTime - coolNote.strumTime) < forbiddenDistance)
		...
		if (!note.wasGoodHit)
		{
			...
			if (!wasHalo)
			{
				...
				if (!note.isSustainNote)
				{
					...
					var addedScoreText:FlxText = new FlxText(0, 0, 0, "(+0)", 24);
					addedScoreText.setFormat(GameData.globalFont, 24, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
					addedScoreText.text = '(+${ScoreFunctions.calculateNote(daRating, noteDiff)})';
					addedScoreText.screenCenter();
					addedScoreText.x = playerStrums.members[note.noteData].x;
					addedScoreText.y = playerStrums.members[note.noteData].y + 40;
					addedScoreText.cameras = [camHUD];
					addedScoreText.font = curStage.startsWith('school') ? Paths.font('vcr') : GameData.globalFont;
					add(addedScoreText);
					...
					FlxTween.tween(addedScoreText, {alpha: 0, y: addedScoreText.y + pos}, Conductor.stepCrochet * 0.002, {
						onComplete: function(twn:FlxTween)
						{
							addedScoreText.destroy();
						}
				...
				if (curSong == 'South' && !note.isSustainNote)
					flashLight(false, note.noteData);
				...
				if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection
					...
					&& curSong == 'Dadbattle')
				{
					FlxTween.cancelTweensOf(lightsDadbattle.members[note.noteData]);
					var colors:Array<FlxColor> = [
						FlxColor.fromRGB(194, 75, 153, 102), FlxColor.fromRGB(0, 255, 255, 102),
						  FlxColor.fromRGB(18, 250, 0, 102), FlxColor.fromRGB(249, 57, 63, 102)
					];
					FlxTween.color(lightsDadbattle.members[note.noteData], 0.2, FlxColor.fromRGB(255, 255, 255, 102), colors[note.noteData]);
					lightsDadbattle.members[note.noteData].alpha = 0.4;
				}
			}
	...
	function appearLights():Void
	{
		...
		if (didLight > colors.length)
			return;

		lightsDadbattle.members[FlxMath.wrap(didLight, 0, 3)].color = FlxColor.WHITE;
		FlxTween.tween(lightsDadbattle.members[didLight], {alpha: 0.4}, {ease: FlxEase.quintOut});
		FlxTween.color(lightsDadbattle.members[didLight], 0.6, lightsDadbattle.members[didLight].color, colors[didLight], {ease: FlxEase.quintOut});
		didLight++;
	}

	var glowSprite:FlxSprite = new FlxSprite().loadGraphic(Paths.image('glowLight'));

	function flashLight(flipped:Bool = false, data:Int = 0):Void
	{
		var colors:Array<FlxColor> = [
			FlxColor.fromRGB(194, 75, 153, 102), FlxColor.fromRGB(0, 255, 255, 102),
			  FlxColor.fromRGB(18, 250, 0, 102), FlxColor.fromRGB(249, 57, 63, 102)
		...
		var flashySprite:FlxSprite = glowSprite.clone();
		flashySprite.cameras = [camHUD];
		flashySprite.color = colors[data];
		flashySprite.flipX = flipped;
		flashySprite.alpha = 0.6;
		insert(members.indexOf(vignette), flashySprite);
		FlxTween.tween(flashySprite, {alpha: 0}, Conductor.crochet / 1000, {
			ease: FlxEase.quadOut,
			onComplete: function(twn:FlxTween)
			{
				flashySprite.destroy();
			}
		});
	}
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'milf' && curBeat >= 168 && curBeat < 200 && camZooming && FlxG.camera.zoom < 1.3 && forceBeat.length < 1)
		{
			milfActive = true;
			FlxG.camera.zoom += 0.015 * (FlxG.save.data.musicVolume / 100);
			camHUD.zoom += 0.03 * (FlxG.save.data.musicVolume / 100);
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && curBeat % camBeat == 0 && !milfActive && notes.length > 1)
		{
			if (forceBeat.length < 1)
			{
				FlxG.camera.zoom += 0.015 * (FlxG.save.data.musicVolume / 100);
		...
		if (curSong == 'Dadbattle')
		{
			if (curBeat == 64)
			{
				FlxTween.tween(darkBG, {alpha: 0.8}, 0.9, {
					ease: FlxEase.quadOut,
					onComplete: function(twn:FlxTween)
					{
						FlxTween.tween(darkBG, {alpha: 0.6}, 0.9, {ease: FlxEase.quadIn, startDelay: 0.4});
					}
				});
			}
			if (curBeat == 96)
			{
				var clonedBoyfriend:FlxSprite = boyfriend.clone();
				clonedBoyfriend.scale.set(boyfriend.scale.x, boyfriend.scale.y);
				clonedBoyfriend.updateHitbox();
				clonedBoyfriend.setPosition(boyfriend.x, boyfriend.y);
				clonedBoyfriend.origin.set(boyfriend.origin.x, boyfriend.y);
				clonedBoyfriend.alpha = 0.3;
				clonedBoyfriend.setColorTransform(1, 1, 1, 1, 255, 255, 255, 255);
				clonedBoyfriend.antialiasing = true;
				insert(members.indexOf(boyfriend), clonedBoyfriend);
				FlxTween.tween(clonedBoyfriend, {alpha: 0}, 2.4, {ease: FlxEase.quadOut});
				FlxTween.tween(clonedBoyfriend.scale, {x: 2, y: 2}, 3, {
					ease: FlxEase.quadOut,
					onComplete: function(twn:FlxTween)
					{
						clonedBoyfriend.destroy();
					}
				});
			}
			...
			if (curBeat == 128)
			{
				FlxTween.tween(darkBG, {alpha: 0}, 0.35, {ease: FlxEase.quadOut});
				milfActive = false;
				camBeat = 2;
			}
			if (curBeat == 120 || curBeat == 122 || curBeat == 124 || curBeat == 126)
			{
				// this is so fucking dumb
				if (curBeat == 120)
					lightsDadbattle.members[0].alpha = 1;
				if (curBeat == 122)
					lightsDadbattle.members[1].alpha = 1;
				if (curBeat == 124)
					lightsDadbattle.members[2].alpha = 1;
				if (curBeat == 126)
					lightsDadbattle.members[3].alpha = 1;

				appearLights();
				FlxTween.tween(this, {defaultCamZoom: 0.8}, 7.5, {ease: FlxEase.quintOut});
			}
			if (curBeat == 176)
				defaultCamZoom = 0.95;
			if (curBeat > 176 && curBeat < 192)
			{
				FlxG.camera.zoom -= 0.015 * (FlxG.save.data.musicVolume / 100);
				camHUD.zoom -= 0.03 * (FlxG.save.data.musicVolume / 100);
				milfActive = true;
			...
			if (curBeat == 384)
			{
				for (i in 0...3)
				{
					FlxTween.tween(lightsDadbattle.members[i], {alpha: 0}, 6, {ease: FlxEase.quadOut});
				}
				// smh
				FlxTween.tween(lightsDadbattle.members[3], {alpha: 0}, 6, {ease: FlxEase.quadOut});
			}
		}
